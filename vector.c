...
{
    if (SIZE_MAX - vector->length < amount)
        return false;
    ...
    if (vector->start == NULL || (vector->capacity < minimumCapacity && !vectorSetCapacity(vector, newCapacity)) || items == NULL)
        return false;
    char *dest = (char *)vector->start + vector->length * vector->elementSize;
...
{
    if (vector->length == 0 || vector->start == NULL)
        return NULL;
    void *source = (char *)vector->start + --vector->length * vector->elementSize;
    if (out != NULL)
        memcpy(out, source, vector->elementSize);
    return out;
}
...
{
    if (destination == NULL || source == NULL || destination->start == NULL || source->start == NULL || destination->elementSize != source->elementSize || destination->start == source->start)
        return false;

    if (SIZE_MAX - destination->length < source->length)
        return false;
    size_t newCapacity = destination->length + source->length;
    if (!vectorSetCapacity(destination, newCapacity))
        return false;
    memcpy((char *)destination->start + destination->length * destination->elementSize, source->start, source->length * source->elementSize);
