...
static bool binaryTreeNodeInitialize(BinaryTreeNode *node, const void *value,
                                     size_t count, size_t elementSize) {
    if (node == NULL) return false;

    void *ownedValue = malloc(elementSize);
    if (ownedValue == NULL) return false;
...
static BinaryTreeNode **chooseNode(BinaryTreeNode **node, const void *item,
...
static BinaryTreeNode *const *binaryTreeNodeSearch(const BinarySearchTree *tree,
                                                   const void *item) {
    if (tree == NULL || tree->root == NULL || item == NULL) return NULL;

    BinaryTreeNode *const *result = &tree->root;
    while (!(result == NULL || *result == NULL)) {
        int comparisonResult = tree->comparator(item, (*result)->value);
        if (comparisonResult == 0) return result;

        result = (BinaryTreeNode *const *)chooseNode((BinaryTreeNode **)result,
...
static void binaryTreeNodeApplyRecursive(BinaryTreeNode *node,
                                         void (*function)(void *element,
                                                          size_t count)) {
    if (node == NULL || function == NULL) return;
...
static BinaryTreeNode *binaryTreeNodeMapRecursive(
    BinaryTreeNode *node, void *(*function)(void *element, size_t count),
    size_t elementSize) {
    if (node == NULL || function == NULL) return NULL;

    BinaryTreeNode *result = malloc(sizeof *result);
    if (result == NULL) return NULL;
    binaryTreeNodeInitialize(result, function(node->value, node->count),
                             node->count, elementSize);
    if (node->left == NULL) {
        result->left = NULL;
    } else {
        result->left =
            binaryTreeNodeMapRecursive(node->left, function, elementSize);
    ...
    if (node->right == NULL) {
        ...
        result->right =
            binaryTreeNodeMapRecursive(node->right, function, elementSize);
...
static Vector *binaryTreeNodeToVectorRecursive(BinaryTreeNode *node,
                                               size_t elementSize) {
    ...
    if (node->right != NULL) {
        Vector *right =
            binaryTreeNodeToVectorRecursive(node->right, elementSize);
        if (right == NULL) {
            vectorFree(vector);
            free(vector);
            return NULL;
...
bool binarySearchTreeInitialize(BinarySearchTree *tree, size_t elementSize,
...
bool binarySearchTreeInsert(BinarySearchTree *tree, const void *item) {
    if (tree == NULL || item == NULL || tree->comparator == NULL) return false;
    if (tree->root == NULL) {
    ...
    while (true) {
        ...
        BinaryTreeNode **childNodePtr =
            chooseNode(&node, item, comparisonResult);
        if (childNodePtr == NULL) return false;
        ...
        if (childNode == NULL) {
            *childNodePtr = malloc(sizeof **childNodePtr);
            ...
            if (!binaryTreeNodeInitialize(newChild, item, 1,
                                          tree->elementSize)) {
                free(newChild);
                return false;
            }
...
void *binarySearchTreeRemove(BinarySearchTree *tree, const void *item) {
    ...
    BinaryTreeNode **nodeToRemove =
        (BinaryTreeNode **)binaryTreeNodeSearch(tree, item);
    if (nodeToRemove == NULL || *nodeToRemove == NULL) return NULL;
    ...
    while ((*nextBiggest)->left != NULL) nextBiggest = &(*nextBiggest)->left;
...
void binarySearchTreeApply(BinarySearchTree *tree,
                           void (*function)(void *element, size_t count)) {
...
String *binarySearchTreeToString(
    BinarySearchTree *tree, String (*elementToString)(const void *element)) {
...
BinarySearchTree *binarySearchTreeMap(
    BinarySearchTree *tree, void *(*function)(void *element, size_t count),
    ...
    if (tree->root != NULL) {
        result->root = binaryTreeNodeMapRecursive(tree->root, function,
                                                  result->elementSize);
